...
def nabla(x, r):
    res = []
    res.append(2 * x[0] + x[1] + 1 + 4 * r * (4 * x[0] + x[1] - 2))
    res.append(8 * x[1] + x[0] + r * (4 * x[0] + x[1] - 2))
    return res

def fi(x, t, r):
    nabla_res = nabla(x, r)
    return (x[0] - t * nabla_res[0]) ** 2 + 4 * (x[1] - t * nabla_res[1]) ** 2 + (x[0] - t * nabla_res[0]) * (x[1] - t * nabla_res[1]) + (x[0] - t * nabla_res[0]) + (r / 2) * (4 * (x[0] - t * nabla_res[0]) + (x[1] - t * nabla_res[1]) - 2)
